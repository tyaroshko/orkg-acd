...
def do_spc(config=None, outfile=None, initems=None, inedgelist=None):
    t0 = time.time()
    ...
    log_file_path = os.path.join(
        data_dir, conf.get("008_search_path_count", "log_file_name")
    )
    ...
    max_citation_net_nodes = conf.getint(
        "008_search_path_count", "max_citation_net_nodes"
    )
    ...
    if inedgelist and inedgelist == "resume":
        ...
        """
        citation_net = nx.read_edgelist(
            file_path_edge_list, create_using=nx.DiGraph(), nodetype=str
        )
        ...
        """
        citation_net = nx.read_edgelist(
            inedgelist, create_using=nx.DiGraph(), nodetype=str
        )
    else:
        """
        ...
        log(
            (
                "raw network is connected = ",
                nx.is_connected(citation_net.to_undirected()),
            )
        )
    ...
    # print(n_top_paths, len(sorted_weights))
    # weight_min = sorted_weights[n_top_paths]
    # weight_min = sorted_weights[min(n_top_paths, len(sorted_weights)-1)]
    print(n_top_paths)
    weight_min = sorted_weights[n_top_paths]
    ...
    selected_item_ids = set(
        [
            spc.__paper_of(str(nd))
            for nd in node_weights
            if node_weights[spc.__paper_of(str(nd))] >= weight_min
        ]
    )
    ordered_items = sorted(
        [(node_weights[nd], nd) for nd in selected_item_ids], reverse=True
    )
