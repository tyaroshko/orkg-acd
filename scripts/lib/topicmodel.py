...
class io:
    ...
    def load_csv_as_dict(self, filename):
        ...
        csvreader = csv.DictReader(
            csv_input_file, delimiter="\t", quotechar="", quoting=csv.QUOTE_NONE
        )
...
class Model:
    ...
    def coccurences(self, file_path_input, extract_tokens=None):
        norma = 0.0
        random.seed()
        cooccurrence_probability = numpy.zeros(
            (self.word_dictionary_size, self.word_dictionary_size), dtype=numpy.int32
        )
                ...
                for i in range(0, len(words)):
                    ...
                    for j in range(i + 1, len(words)):
                        ...
                        if w1 in self.word_dictionary and w2 in self.word_dictionary:
                            ...
                            cooccurrence_probability[w1_pos][w2_pos] = (
                                cooccurrence_probability[w1_pos][w2_pos] + 1
                            )
                            cooccurrence_probability[w2_pos][
                                w1_pos
                            ] = cooccurrence_probability[w1_pos][w2_pos]
                            norma += 2
                        # else:
    ...
    def word_probability(self, cooccurrence_probability):
        ...
        for i in range(0, self.word_dictionary_size):
            for j in range(0, self.word_dictionary_size):
                word_probability[i] = (
                    cooccurrence_probability[i][j] + word_probability[i]
                )
    ...
    def load_topic_model(self, cooccurrence_probability, topic_model):
        ...
        self.ptw = numpy.zeros(
            (self.n_topics, self.n_words)
        )  # numpy.save('out/TopicModel.npy', {'H':H, 'N':N})

        for iWord in range(0, self.n_words):
            for iTopic in range(0, self.n_topics):
                if self.pw[iWord] > 0:
                    self.ptw[iTopic][iWord] = (
                        self.pwt[iWord][iTopic] * self.pt[iTopic] / self.pw[iWord]
                    )
