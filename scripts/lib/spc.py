...
def spc(citation_net):
    ...
    while n_updates > 0:
        ...
        for nd in n_minus:
            if n_minus[nd] < 0:
                in_edges = [
                    n_minus[ed] for (ed, _) in list(citation_net.in_edges([nd]))
                ]
    ...
    while n_updates > 0:
        ...
        for nd in n_plus:
            if n_plus[nd] < 0:
                out_edges = [
                    n_plus[ed] for (_, ed) in list(citation_net.out_edges([nd]))
                ]
    ...
    return {"node_weights": node_weighs, "edge_weights": edge_weights}
