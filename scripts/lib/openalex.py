...
class Api:
    ...
    def load_by_ids(self, entry_ids, verbose=False):
        restrict_ids = "ids.openalex:" + (
            "|".join([self.short_id(x) for x in entry_ids])
        )
        if len(self.include_topics) > 0:
            restrict_topics = ",concepts.id:" + (
                "|".join([self.short_id(x) for x in self.include_topics])
            )
        else:
            restrict_topics = ""
    ...
    def load_entity(self, entity):
        ...
        for raw_author in raw_authors:
            ...
            a.affiliation = get_optional(
                ["institutions", 0, "display_name"], raw_author, ""
            )
        ...
        res_dict["venue_full_name"] = get_optional(
            ["host_venue", "display_name"], entity, []
        )
    ...
    def load_by_rids_extended(self, entry_ids, verbose=False):
        ...
        sbIds = (
            "or("
            + (",".join(["RId=" + regex.sub("", str(x)) for x in entry_ids]))
            + ")"
        )
        sbFIds = (
            "or("
            + (
                ",".join(
                    [
                        "Composite(F.FId=" + regex.sub("", str(id)) + ")"
                        for id in self.include_topics
                    ]
                )
            )
            + ")"
        ...
        return self.call_api(
            "and(" + sbIds + ", " + sbFIds + ")", ",".join(Api.FIELDS), verbose
        )
