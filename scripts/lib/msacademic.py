...
class Api:
    ...
    def load_by_ids(self, entry_ids, verbose=False):
        ...
        sbIds = (
            "or("
            + (",".join(["Id=" + regex.sub("", str(int(x))) for x in entry_ids]))
            + ")"
        )
        sbFIds = (
            "or("
    ...
    def call_api(self, expr, attributes, verbose=False):
        ...
        if verbose:
            print("self.subscriptionKey=" + self.subscription_key + ";")
        # print expr
            ...
            for en in refs:
                ...
                if topicIsValid and contentIsValid:
                    ...
                    for newId in en.referencesTo:
                        if (
                            (newId not in msAcademicIndexedIds)
                            and (newId not in msAcademicInvalidIds)
                            and (newId not in msAcademicQueueIds)
                        ):
                ...
                print(
                    str(counter)
                    + " of "
                    + str(len(msAcademicQueue))
                    + " : "
                    + str(en.entryId)
                    + "  "
                    + str(en.entryPublished)
                    + "  ",
                    en.entryTitle.encode("utf-8"),
                )
