...
def do_clear_terms(config=None, dir_in_terms=None, dir_out_terms=None, trace=0):  #
    """
    :param config:
    ...
    partial_term_files = sorted(
        [
            join(dir_in_terms, f)
            for f in listdir(dir_in_terms)
            if isfile(join(dir_in_terms, f)) and f.lower().endswith(".txt")
        ]
    )
    ...
    for partial_term_file in partial_term_files:
        ...
        file_counter += 1
        terms_file_path = join(
            dir_out_terms,
            "T" + (("00000000000000000" + str(file_counter))[-10:]) + ".txt",
        )
