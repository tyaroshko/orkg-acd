...
def snowball(
    ...
    log_file_path = os.path.join(
        data_dir, conf.get("007_restricted_snowball", "log_file_name")
    )
    ...
    file_path_queued_ids = os.path.join(
        data_dir, conf.get("007_restricted_snowball", "file_path_queued_ids")
    )  # queued items
    file_path_seed_ids = os.path.join(
        data_dir, conf.get("007_restricted_snowball", "file_path_seed_ids")
    )  # seed item ids
    ...
    # =====================================================
    # load known ids
    file_path_known_ids = os.path.join(
        data_dir, conf.get("007_restricted_snowball", "file_path_known_ids")
    ...
    # =====================================================
    # load done ids
    file_path_done_ids = os.path.join(
        data_dir, conf.get("007_restricted_snowball", "file_path_done_ids")
    )  # items that were in the queue
    done_ids = set()
        ...
        for item in items:
            ...
            item["tokens"].extend(
                ct.exclude_unknown_tokens(
                    ct.get_tokens(str(item["title"]) + ". " + str(item["abstract"]))
                )
            )
            # /get tokens
            # -----------------------------------------------------------------
            ...
            if item_is_valid:
                ...
                for related_entry_id in item["references_to"]:
                    if (
                        related_entry_id not in done_ids
                        and related_entry_id not in queued_ids_set
                    ):
